 ...
 * Marketplace Service
 *
 * STATUS — FULLY MOCKED (clearly labelled per assignment brief):
 *   - All 10 products are dummy Kerala Ayurveda catalogue items.
 *   - Images use Unsplash placeholder URLs scoped to relevant herb/wellness queries.
 *   - Prices are illustrative INR figures (in paise); not from live Shopify data.
 *   - Ratings and review counts are invented.
 *   - In production this would query the Shopify Storefront API (products + collections)
 *     with Redis cache and a short TTL.
 *
 * Business logic (filtering, sorting, featured flag) is in this service,
 * not in the controller, so it can be tested and extended independently.
 */
...
export interface MarketplaceVariant {
  id: string;
  title: string;            // e.g. "60 Capsules"
  priceCents: number;       // INR paise
  priceDisplay: string;     // e.g. "₹2,916"
  available: boolean;
}
...
export interface MarketplaceProduct {
  ...
  imageUrl: string;
  /** Low-res blur placeholder (base64 data URI or tiny URL) — for lazy loading UX */
...
// Unsplash source URLs — deterministic, no API key needed.
// Each photo is picked to visually represent the herb/product category.
const IMG = (id: string) =>
  `https://images.unsplash.com/photo-${id}?auto=format&fit=crop&w=600&q=80`;

// ── Product catalogue (10 items) ───────────────────────────────────────────────

const CATALOGUE: MarketplaceProduct[] = [
  // 1 — Ashwagandha (existing PDP product, also shown in marketplace)
  {
    handle: 'ashwagandha',
...
export class MarketplaceService {
  /**
   * Returns the full marketplace product list.
   * Optional `category` filter; optional `featured` flag filter.
   */
  getProducts(filters?: {
    category?: ProductCategory;
  ...
  /**
   * Returns a single marketplace product by handle.
   */
